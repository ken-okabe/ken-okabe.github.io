...
// 現在は、: `en` と `ja` の二言語だけをサポートしている。

// サポートする言語の追加は、比較的容易ですが、1点だけ**重要な改修**が必要になります。

// ## 容易な点 🟢

// スクリプトの基本的な構造は、言語の追加を想定しています。

// 1.  **設定の追加**: `SUPPORTED_LANGS` という配列に `'kr'` などの新しい言語コードを追加するだけで、スクリプトは新しい言語を認識します。
    
// 2.  **ループ処理**: `en` と `ja` を個別に扱っている箇所の多くは、`SUPPORTED_LANGS` 配列をループする処理に簡単に変更できます。
    
// 3.  **正規表現**: `:::lang-(en|ja)` の部分は、`SUPPORTED_LANGS` 配列から動的に `(en|ja|kr|...)` のような形を生成するように変更できます。
    

// ## 検討が必要な点 🟡

// 最も大きな変更点は、**フォールバックのルールを再定義**する必要があることです。

// 現在のルールは、「`en`がなければ`ja`を使い、`ja`がなければ`en`を使う」という2言語間のシンプルなものです。

// 3言語以上になった場合、例えば`en`と`kr`はあるが`ja`がない時に、`ja`にはどちらの内容を割り当てるべきか、という新しいルールを決める必要があります。一般的には、**「特定の言語（例: 英語）をデフォルトとし、存在しない言語はすべてそのデフォルト言語の内容で埋める」**といった仕様に改修することになります。

// **結論として**、基本的な仕組みは拡張しやすいですが、3言語以上になった場合の「フォールバック」のルールをどうするか、という仕様の決定が最も重要な鍵となります。
